# --- Syringe Workflow Detection Script (Dual Video File Mode - Simplified Logic v2 - No Timeouts, Persistent Vol Check) ---
# Processes two synchronized video files to detect syringes, estimate volume, track them,
# and validate a workflow based on simple disappearance/appearance events.
# Assumes videos start at the exact same time.
# Volume is checked PERSISTENTLY after insertion until found.
# Program attempts to exit automatically after a successful return cycle or when videos end.
...
class SyringeVolumeEstimator:
    """
    Handles detection, tracking, and volume estimation of syringes using YOLO Pose for a SINGLE camera stream/video file.
    ...
    def draw_fps_counter(self, frame: np.ndarray, prefix: str = "Processing FPS") -> None:
        """Draws the processing FPS, not the source video FPS."""
        current_timestamp = time.monotonic(); self.last_timestamps.append(current_timestamp)
        if len(self.last_timestamps) > 1:
            try:
                time_diffs = np.diff(list(self.last_timestamps)); valid_diffs = time_diffs[time_diffs < 1.0] # Use monotonic time difference
            ...
            except Exception as e: print(f"Warning: Error calculating processing FPS: {e}")
    ...
    def process_frame( self, frame: np.ndarray, timestamp: float, writer: csv.writer = None, camera_source: str = "unknown") -> tuple[np.ndarray, list]:
        if frame is None:
             print(f"Warning: Received None frame for processing ({camera_source}). Skipping.")
             return np.zeros((100, 100, 3), dtype=np.uint8), [] # Return a dummy frame
        ...
        fps_prefix=f"{camera_source.capitalize()} Proc FPS"; self.draw_fps_counter(annotated_frame, prefix=fps_prefix) # Label change
            ...
            if (results_data.keypoints is not None and len(results_data.keypoints.xy)>i and results_data.keypoints.conf is not None and len(results_data.keypoints.conf)>i and results_data.keypoints.xy[i].shape[0]>=4):
                try:
                    ...
                    if DEBUG_MODE and camera_source == 'manikin':
                        ...
                        print(f"DEBUG (Manikin Vid) ID: {track_id}, Kpts Conf: [{kpts_conf_str}], width_px: {width_px:.2f}, height_px: {height_px:.2f}")
                    # --- DEBUG LOGGING END ---
                    ...
                    if width_px > 2 and height_px > 2: # Check pixel dimensions
                        for D_cm in self.possible_diameters:
                            ...
                            if DEBUG_MODE and camera_source == 'manikin': # and D_cm == 1.0: # Optionally focus on correct diameter
                                 print(f"DEBUG (Manikin Vid) ID: {track_id}, Diam: {D_cm:.2f}, H_cm: {H_cm:.2f}")
                            # --- DEBUG LOGGING END ---
                            if 0<H_cm<=30.0: # Use 30cm limit
                                radius_cm=D_cm/2.0
                 ...
                 if DEBUG_MODE and camera_source == 'manikin':
                      ...
                      print(f"DEBUG (Manikin Vid) ID: {track_id}, Keypoints insufficient/missing ({kp_info})")
                 # --- DEBUG LOGGING END ---
    ...
    def setup_video_writer( self, output_path: str, frame_width: int, frame_height: int, fps: float) -> cv2.VideoWriter:
        # save_video flag is now controlled externally in the main script
        if fps <= 0: fps = 25.0; print(f"Warning: Invalid FPS ({fps:.1f}) retrieved from input video. Using default: {fps:.1f} FPS for writer.")
        if frame_width <= 0 or frame_height <= 0: print(f"Error: Invalid frame dimensions ({frame_width}x{frame_height}) from input video. Cannot create video writer."); return None
        ...
        try:
            ...
            return out
        except Exception as e: print(f"Error initializing VideoWriter for {output_path}: {e}. Video saving disabled for this stream."); return None
...
# --- THIS CLASS REMAINS LARGELY UNCHANGED, as it operates on detections ---
class SyringeTestWorkflow:
    ...
    def _log_header(self):
        self._log("--- Workflow Initialized (Video File Mode - Simplified Logic v2 - No Timeout / Persistent Vol) ---", level="SETUP") # Updated header
        self._log(f"Table Zones (Syringes Video): {list(self.table_zones)}", level="SETUP")
        self._log(f"Target Zones (Manikin Video): {list(self.target_zones)}", level="SETUP")
        ...
        self._log("--- Processing Video Files ---", level="SETUP") # Updated header
    ...
    def _reset_state(self, reason: str):
        ...
        if self.current_state != self.STATE_IDLE:
            if self.active_syringe_id is not None:
                ...
                summary = f"Cycle ended for syringe ID {id_to_report} (from {source_cam} video)." # Updated text
    ...
    def _update_tracking_history(self, all_detections, current_time):
        ...
        for det in all_detections:
            source = det.get('camera_source', 'unknown') # Uses 'manikin' or 'syringes' now
            track_id = det.get('id')
            zone = det.get('zone')
    ...
    def _initialize_table_state(self, syringe_detections):
        ...
        if DEBUG_MODE: print("DEBUG: Performing initial table scan (from video)...")
        temp_table_state = {}
        ...
        if len(temp_table_state) >= required_count :
             ...
             self._log("Initial table state identified from video.", level="INFO") # Updated text
             if DEBUG_MODE: print(f"DEBUG: Initial table state set: {self.syringes_on_table}")
        elif DEBUG_MODE: print(f"DEBUG: Initial scan found only {len(temp_table_state)}. Waiting for more frames...")
    ...
    def update_state(self, all_detections: list[dict], current_time: float):
        ...
        if self.current_state == self.STATE_IDLE:
            ...
            if len(missing_ids) == 1:
                ...
                self._log(f"Syringe ID {pickup_id} (syringes vid) disappeared from '{origin_zone}'. State -> {self.STATE_SYRINGE_PICKED}.") # Updated text
            ...
            pickup_id = self.active_syringe_id # ID from syringe video
            ...
            if insertion_detected:
                insertion_id = insertion_detected['id'] # Manikin video ID
                insertion_zone = insertion_detected['zone']
                self._log(f"Syringe detected in target zone '{insertion_zone}' (Manikin Vid ID: {insertion_id}). Assuming insertion of pickup ID {pickup_id}. State -> {self.STATE_SYRINGE_INSERTED}.") # Updated text
            ...
            inserted_id = self.active_syringe_id # ID from manikin video
            ...
            if return_detected:
                returned_id = return_detected['id'] # Syringe video ID
...
if __name__ == "__main__":
    ...
    print("--- Syringe Workflow Test Initializing (Dual Video File Mode - Simplified Logic v2 - No Timeout / Persistent Vol) ---") # Updated Title
    ...
    # --- VIDEO FILE INPUT ---
    MANIKIN_VIDEO_PATH = "/Users/andreas/Desktop/repos/masterthesis/volume_estimation/recordings/cam1_20250510_140711_2592x1944.mp4"  # *** EDIT HERE: Path to the manikin perspective video ***
    SYRINGES_VIDEO_PATH = "/Users/andreas/Desktop/repos/masterthesis/volume_estimation/recordings/cam0_20250510_140711_2592x1944.mp4" # *** EDIT HERE: Path to the table/syringes perspective video ***
    ...
    MANIKIN_ZONE_DEFINITIONS = [
        ActiveZone(name="Arm", rect=(1800, 1150, 2000, 1400)),
    ...
    TARGET_VOLUME_ML = 10                     # *** EDIT HERE ***
    VOLUME_TOLERANCE_ML = 5                  # *** EDIT HERE ***
    CORRECT_TARGET_ZONE = "Throat"             # *** EDIT HERE ***

    # --- Timeouts Removed ---

    # --- Output Files ---
    SAVE_OUTPUT_VIDEO = True # *** EDIT HERE: Set to False to disable saving processed videos ***
    OUTPUT_VIDEO_PATH_MANIKIN = "manikin_processed_video.mp4" # Output file name
    OUTPUT_VIDEO_PATH_SYRINGES = "syringes_processed_video.mp4" # Output file name
    RAW_CSV_PATH = 'syringe_volume_data_raw_combined_video.csv' # Output file name
    WORKFLOW_LOG_PATH = 'syringe_test_workflow_log_video.txt' # Output file name
    ...
    if not os.path.exists(MANIKIN_VIDEO_PATH): print(f"FATAL: Manikin video file not found: {MANIKIN_VIDEO_PATH}"); sys.exit(1)
    if not os.path.exists(SYRINGES_VIDEO_PATH): print(f"FATAL: Syringes video file not found: {SYRINGES_VIDEO_PATH}"); sys.exit(1)
    if MANIKIN_VIDEO_PATH == SYRINGES_VIDEO_PATH: print(f"FATAL: Manikin and Syringes video paths are the same."); sys.exit(1)
    ...
    try:
        ...
        if not cap_syringes.isOpened(): raise IOError(f"Cannot open syringes video file: {SYRINGES_VIDEO_PATH}")

        # Get Video Properties (Width, Height, FPS)
        manikin_w=int(cap_manikin.get(cv2.CAP_PROP_FRAME_WIDTH)); manikin_h=int(cap_manikin.get(cv2.CAP_PROP_FRAME_HEIGHT)); manikin_fps=cap_manikin.get(cv2.CAP_PROP_FPS)
        print(f"Manikin video ({MANIKIN_VIDEO_PATH}): {manikin_w}x{manikin_h} @ {manikin_fps:.2f} FPS")
        syringes_w=int(cap_syringes.get(cv2.CAP_PROP_FRAME_WIDTH)); syringes_h=int(cap_syringes.get(cv2.CAP_PROP_FRAME_HEIGHT)); syringes_fps=cap_syringes.get(cv2.CAP_PROP_FPS)
        print(f"Syringes video ({SYRINGES_VIDEO_PATH}): {syringes_w}x{syringes_h} @ {syringes_fps:.2f} FPS")

        # Setup Video Writers (using retrieved properties)
        if SAVE_OUTPUT_VIDEO:
            print("Setting up video writers...")
            # Note: Estimator's setup_video_writer now uses the passed fps/dims
            ...
            # Set the save_video flag based on successful writer creation
            estimator_manikin.save_video = (out_manikin is not None)
            estimator_syringes.save_video = (out_syringes is not None)
        ...
        print("\n--- Starting Main Loop (Processing video files, press 'q' to quit early) ---"); frame_count=0; start_time=time.monotonic()
        ...
        while True:
            ...
            # Stop if EITHER video ends (assuming synchronization)
            if not ret_m or not ret_s:
                print("\n---")
                if not ret_m: print("Manikin video ended.")
                if not ret_s: print("Syringes video ended.")
                print("End of video file(s) reached. Exiting loop.")
                print("---")
                break
            # --------------------------------
            ...
            # Process frames
            annotated_frame_m, detections_m = [], [] # Initialize in case of errors
            annotated_frame_s, detections_s = [], []
            try:
                annotated_frame_m, detections_m = estimator_manikin.process_frame(frame_m, timestamp, writer, 'manikin')
            except Exception as e:
                 print(f"Error processing manikin frame {frame_count}: {e}")
                 traceback.print_exc() # Print detailed traceback for debugging
                 annotated_frame_m = frame_m if frame_m is not None else np.zeros((manikin_h, manikin_w, 3), dtype=np.uint8) # Show raw frame or black frame
            ...
            try:
                annotated_frame_s, detections_s = estimator_syringes.process_frame(frame_s, timestamp, writer, 'syringes')
                 ...
                 print(f"Error processing syringes frame {frame_count}: {e}")
                 traceback.print_exc() # Print detailed traceback for debugging
                 annotated_frame_s = frame_s if frame_s is not None else np.zeros((syringes_h, syringes_w, 3), dtype=np.uint8) # Show raw frame or black frame
            ...
            try:
                 ...
                 if annotated_frame_m is None or annotated_frame_m.size == 0 or annotated_frame_s is None or annotated_frame_s.size == 0:
                     print(f"Warning: Skipping display for frame {frame_count} due to invalid frame(s).")
                     continue
                 ...
                 # Handle potential zero height issues if errors occur
                 display_m = annotated_frame_m
                 if h_m != max_h and h_m > 0:
                     display_m=cv2.resize(annotated_frame_m,(int(w_m*max_h/h_m),max_h))

                 display_s = annotated_frame_s
                 if h_s != max_h and h_s > 0:
                     display_s=cv2.resize(annotated_frame_s,(int(w_s*max_h/h_s),max_h))
                 ...
                 cv2.imshow('Syringe Workflow (Video Files) - Press Q to Quit', combined)
            except Exception as e:
                print(f"Display error on frame {frame_count}: {e}")
                # Attempt to show individual frames if stacking fails
                try:
                     if annotated_frame_m is not None and annotated_frame_m.size > 0:
                         cv2.imshow('Manikin Video', cv2.resize(annotated_frame_m, (annotated_frame_m.shape[1]//2, annotated_frame_m.shape[0]//2))) # Smaller resize
                except Exception as e_disp_m: print(f" Error displaying manikin frame individually: {e_disp_m}")
                try:
                     if annotated_frame_s is not None and annotated_frame_s.size > 0:
                          cv2.imshow('Syringes Video', cv2.resize(annotated_frame_s, (annotated_frame_s.shape[1]//2, annotated_frame_s.shape[0]//2))) # Smaller resize
                except Exception as e_disp_s: print(f" Error displaying syringes frame individually: {e_disp_s}")


            # Quit Condition
            if cv2.waitKey(1) & 0xFF == ord('q'): # waitKey(1) is important for video playback speed
                print("'q' pressed. Exiting loop early.")
                break
        ...
        if cap_manikin: cap_manikin.release(); print("Manikin video capture released.")
        if cap_syringes: cap_syringes.release(); print("Syringes video capture released.")
        if out_manikin: out_manikin.release(); print("Manikin video writer released.")
        if out_syringes: out_syringes.release(); print("Syringes video writer released.")
